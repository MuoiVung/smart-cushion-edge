 ...
 *
 * Role        : Left Side + Temp Sensor + Vibration Actuator
...
#define PIN_MOTOR 13  // PWM motor output (LEDC channel 0)
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  if (strlen(target) > 0 && strcmp(target, "esp32-1") != 0)
    return;
  ...
  if (active) {
    if (strlen(pattern) > 0 && strcmp(pattern, "continuous") != 0) {
...
void runPattern(const char *pattern, int intensity) {
  ...
  if (strcmp(pattern, "short_triple") == 0) {
    ...
    setMotor(true, intensity);
    delay(2000); // Đổi từ 800ms thành 2000ms (2s) theo yêu cầu
    setMotor(false);
  } else {
